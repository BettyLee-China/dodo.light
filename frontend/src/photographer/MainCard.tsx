...
const MainCard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setBirthday(storeBirthday ? dayjs(storeBirthday).format('YYYY-MM-DD') : null);
    setBio(typeof storeBio === 'string' ? storeBio : '');
  }, [storeNickname, storeGender, storeBirthday, storeBio]);
  ...
  const handleCrop = async () => {
    ...
    if (result) {
      ...
      if (userId) {
        useUserProfileStore.getState().uploadAvatar(userId, result.file);
  ...
  const updateField = async (field: string, value: string) => {
    ...
    try {
      await modifyProfile(userId, formData);
